...
import {MockDSCEngine} from "../mocks/mockSrc/MockDSCEngine.sol";
...
contract DSCEngineTest is Test {
    ...
    function setUp() public {
        ...
        (dsce, helperConfig) = deployer.run();
        dsc = dsce.i_dsc();
    ...
    function testRevertsIfTokenLengthDoesntMatchPriceFeeds() public {
        ...
        new DSCEngine(tokenAddresses, feedAddresses);
    }
    ...
    function testGetTokenAmountFromUsd() public view {
        ...
        // 2000 / 1 = 100 / x -> 100 / 2000 = 0.05 weth
    ...
    function testEmitCollateralDeposited() public {
        ...
        emit CollateralDeposited(user, weth, 10 ether); // (1e19 == 10e18 == 10 ether)
        dsce.depositCollateral(weth, amountCollateral);
        vm.stopPrank();
    }
    ...
    function testRevertsWhenSendingInsufficientBalance() public {
        vm.startPrank(user);
        ERC20Mock(weth).approve(address(dsce), amountCollateral + 1);
        vm.expectRevert();
        dsce.depositCollateral(weth, amountCollateral + 1);
        vm.stopPrank();
    }
    ...
    function testRevertsIfMintFails() public {
        ...
        vm.prank(owner);
        MockDSCEngine mockDsce = new MockDSCEngine(tokenAddresses, feedAddresses, address(mockDsc));
    ...
    function testRevertsIfMintAmountOfDscIsZero() public depositedCollateral {
        vm.startPrank(user);
        vm.expectRevert(DSCEngine.DSCEngine__NeedsMoreThanZero.selector);
        dsce.mintDsc(0);
        vm.stopPrank();
    ...
    function testRevertsIfMintAmountBreaksHealthFactor() public depositedCollateral {
        ...
        // totalDscMinted/0, collateralValueInUsd(10e18*($2_000e8*1e10)/1e18) = 10e18*$2_000= $20_000e18 wei
    ...
    function testRevertsIfMintedDscBreaksHealthFactor() public {
        (, int256 price,,,) = MockV3Aggregator(ethUsdPriceFeed).latestRoundData();
        // 10e18 * (2000e8 * 10e10) / 10e18 = 20000e18
        amountToMint = (amountCollateral * (uint256(price) * dsce.getAdditionalFeedPrecision())) / dsce.getPrecision();
    ...
    function testCanMintWithDepositCollateral() public depositedCollateralAndMintedDsc {
        uint256 userBalance = dsc.balanceOf(user);
    ...
    function testRevertsIfUnapprovedTokenPass() public {
        ERC20Mock randToken = new ERC20Mock();

        vm.startPrank(user);
        ERC20Mock(weth).approve(address(dsce), amountCollateral);
        vm.expectRevert(abi.encodeWithSelector(DSCEngine.DSCEngine__TokenNotAllowed.selector, address(randToken)));
        dsce.redeemCollateral(address(randToken), amountCollateral);
        vm.stopPrank();
    }
    ...
    function testRevertsIfZeroCollateral() public depositedCollateralAndMintedDsc {
        vm.startPrank(user);
        dsc.approve(address(dsce), amountToMint);
        vm.expectRevert(DSCEngine.DSCEngine__NeedsMoreThanZero.selector);
    ...
    function testRevertsIfUnapprovedToken() public {
        ...
        ERC20Mock(weth).approve(address(dsce), amountCollateral);
        vm.expectRevert(abi.encodeWithSelector(DSCEngine.DSCEngine__TokenNotAllowed.selector, address(randToken)));
        dsce.redeemCollateralForDsc(address(randToken), amountCollateral, 0);
        vm.stopPrank();
    }
